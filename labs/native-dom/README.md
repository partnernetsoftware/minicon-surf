...
| `native-dom-control-0.0.2-surface`, `native-dom-control-0.0.2-surface-attribution`, `native-dom-control-0.0.2-surface-snapshot-attribution` | the host with the G3 surface process, the surface-owned mmap frame region and the court-only stage log, host `32343eb5…` (`surface_sha256` names the child binary); these three are visual runs recorded before the headless rule and are not rerun | the current headless build (host `69778384…`, child `8e9e51e2…`, the replay child's frame-relative bound) carries `native-dom-control-0.0.2-surface-headless` (17/17), `-surface-paired-causal` (arm B only) and the rerun `-profile`, `-frame-realm`, `-https`, `-secure-cookie` receipts; `-cdp-frame-tree` still carries an earlier build (`a69992be…`) because its pinned Node client modules under `target/labs/d4` were removed from this machine and cannot be reinstalled offline, so that court is unrunnable here and its receipt is not refreshed; the surface court is visual and waits for a by-hand opted-in run | the journeys (27/27, 35/35 under both allocators) were rerun on the headless rule build |
