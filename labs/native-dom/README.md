...
- **The realm-probe repair has no passing receipt.** `-probe-truthfulness` is
  committed at **21 of 25**, the arm frozen *before* the repair, on
  `05aa12f7cf6d…`. The repair landed at `a229c13` and the court has read 25 of
  25 on every binary since, but always in a scratch run reported in a commit
  message. Under the receipt convention in `AGENTS.md` that missing evidence is
  a gap, and closing it is a ruling of its own rather than something to backfill
  quietly.
