...
cost returns on close. **A child now compiles only what a child can reach**
(`shim-split-design-0.0.1.md`): `dom_shim_base.js` is the tree, its events,
the selector engine and the seed, and `dom_shim_main.js` — fetch, cookies and
`localStorage`, the location accessors and the intent slot, the window as an
event target with the lifecycle bridge, `queueMicrotask`, timers, `console`
and `navigator` — is compiled only by a realm that runs page script. The
extension reaches the base through a one-shot non-enumerable handle that
deletes itself as it hands its internals over; a child realm gets no
extension and the host deletes the handle itself, refusing the realm if it
survives. That took M1 from 261,961 to **221,657** and M2 from 1,831,451 to
1,549,323 against unmoved caps, with a main-only page costing a few hundred
bytes *less*, so **M1 has 40,486 bytes of headroom** where it had 182. That
margin is a **floor to hold, not a budget**: base growth needs its own proof
that a child's snapshot or action semantics require it.
...
evaluation** so the jobs a handler queues drain before the next begins:
`readyState` becomes `interactive` and `readystatechange` is dispatched at the
document, then `DOMContentLoaded` at the document — it bubbles, and reaches a
listener on the window with the document as `target` and the window as
`currentTarget` — then `readyState` becomes `complete` with another
`readystatechange`, then `load` at the window, which does not bubble. Each
fires exactly once per document and none is cancelable. The window is a
bounded event target: `addEventListener`, `removeEventListener`,
`dispatchEvent` and `onload` as a property, sharing one listener model with
every node, stored in a closure a page cannot reach. The same function
registered twice for the same target and type is registered **once**. The
steps themselves are behind a per-realm capability the host mints before any
page script runs and passes with each call; a call without it, with a wrong
one, out of order or repeated dispatches nothing, so a page can neither forge
the lifecycle nor drive it. An event bubbles to the window only when its path
actually reached the document, so a detached subtree stops at its own
ancestors, and `document.parentNode` stays `null`.

Losses: `defer` and `async` ordering, resource load of any kind, `pageshow`,
`pagehide`, `beforeunload`, `unload`, `visibilitychange`, handler properties
other than `onload`, listener options (`capture`, `once`, `passive`,
...
Events (`event-fidelity-design-0.0.1.md`): one `Event` and one dispatcher,
both in the base, because the base's own `click`, `submit` and `reset` raise
events too and a main-only class would have missed exactly those. An event's
state is closure-owned and keyed by the event: `type`, `bubbles`,
`cancelable`, `composed`, `target`, `currentTarget`, `defaultPrevented`,
`eventPhase` and `dispatching` are read-only to page script, `preventDefault`
is the only door to `defaultPrevented` and only when the event is cancelable,
and the dispatcher is the only writer of the rest. The host reads
`defaultPrevented` back to decide whether an activation proceeds, so that is
an authority boundary and not a nicety: before this slice, a handler that
merely assigned the field cancelled a host-driven navigation. The dispatcher
sets the phase at each hop (2 at the target, 3 while bubbling, 0 outside a
dispatch), does not call a listener added during the dispatch, does not call
one removed during it, honours `stopPropagation` and
`stopImmediatePropagation`, keeps running after a listener throws, refuses an
event already in flight with an `InvalidStateError` — which also ends the
unbounded recursion a re-dispatching handler used to cause — and cleans up on
every path out while leaving `target` as the last target dispatched to.
Focus is the **agent's**, not the page's. The focused element lives in the
base's closure, where no page can reach it, and moves only when the host asks
through the capability bridge — a reserved request the base answers by moving
focus and dispatching nothing, because not every host activation goes through
the dispatcher and hooking the dispatcher would have let a page's own
synthetic click move focus too. `document.activeElement` is exposed by the
main extension through a getter that can report focus and cannot move it, and
defaults to `document.body`. `focus` and `focusin`, `blur` and `focusout` go
through the base's own dispatcher in the standard's order, with only the `in`
and `out` pair bubbling. A page's `focus()` and `blur()` **move nothing and
raise nothing**: a page saying it is focused is not evidence that it is, and
the divergence — a page that focuses itself still reads what the agent last
activated — is chosen over a value the host would be inventing. Losses: no
`tabindex`, `contenteditable`, `autofocus`, Tab or sequential navigation, no
`:focus`, no `hasFocus`, no focus across frames, no `relatedTarget`, and no
focus field in the snapshot.

`Node.cloneNode(deep)` copies element names, attributes and text data,
shallow unless a page asks for deep. It carries **no listeners** — those live
keyed by node where a copy cannot reach them — **no IDL state**, because the
typed `value` and the `checked` property are kept apart from the attributes,
and **no focus**, because that is the host's. The kinds it copies are a closed
set, text and element, and a kind it does not model **fails the whole call**
rather than being skipped, so a copy is faithful or it is an error. A detached
copy moves no revision until the page appends it.

`Element.toggleAttribute(name, force)` adds the attribute with an empty
value, removes it, honours `force` in both directions, and answers whether it
is present afterwards. It goes through `setAttribute` and `removeAttribute`,
so a toggle that changes the attribute moves the revision once and one whose
`force` matches the current state moves nothing. It validates no name, because
its neighbours do not — a space or an empty string is accepted here where a
browser throws `InvalidCharacterError`, and that is recorded as a base
candidate rather than half-fixed in one method.

`Element.getAttributeNames()` answers a new array of the element's attribute
names, lowercased and in insertion order, read from the element's own
attribute map rather than from the `attributes` view — so it allocates nothing
per attribute and claims to be nothing more than an array.

`Element.closest` answers the nearest inclusive ancestor a selector matches,
walking `parentNode` and stopping at the document. It is a **compatibility
fix, not a capability** — a page could always write that walk over `matches`,
and a real page calls `closest` instead and used to die where a browser would
have answered. It refuses exactly what `matches` refuses: child combinators,
commas and pseudo-classes throw, and they throw the selector engine's plain
`Error` rather than a `SyntaxError` `DOMException`, which is a base gap
recorded as its own candidate rather than papered over here.

`dataset` is built when a page reads it, not in every `Element` constructor:
it was a `Proxy`, its handler and three closures on every element of every
document in every realm — about 832 bytes an element, against a node costing
about 2,082 in total — for an API no host script names. The lazy accessor
lives in the main extension with the `kebab` helper that serves nothing else,
so a child realm has no `dataset` at all and the one-shot handle did not widen.

Eleven page-only DOM members — `firstChild`, `lastChild`, `parentElement`,
`appendChild`, `remove`, `innerText`, `defaultValue`, `focus`, `blur`,
`submit` and `contains` — are installed by the main extension, each the base's
own implementation rather than a rewrite. `contains` calls the base's own
helper through the one-shot handle, because that helper is still what gives
`MutationObserver` its subtree scope and one walk is better than two that can
drift.

Three other page-only members
— `matches`, `removeAttribute` and `replaceChildren` — stay in the base by
ruling, because each is written in terms of base internals and moving
them would widen the one-shot handle into a second coupling.

The page-facing view of an event — `isTrusted`, `type`, `bubbles`,
`cancelable`, `composed`, `target`, `currentTarget`, `eventPhase`,
`dispatching` and `timeStamp` — is installed by the main extension on the same
class, reading the same hidden state. A child realm compiles none of it,
because nothing in a child can read it: it runs no page script, and the host
answers from hidden state through its own dispatcher. `defaultPrevented` stays
in the base, and only because `Element.reset` reads it there. That is a real
divergence between a main and a child `Event.prototype` — invisible, not
absent — and it is worth **6,192 bytes of M1 per child**, because what a base
member costs is 600 to 960 bytes each, not its weight in source.

The privileged path is built only from intrinsics captured **before any page
script runs**: `Reflect.apply`, `String`, `Map`, the `WeakMap` and `Map`
prototype methods and the array methods it needs. It invokes a listener
through the captured apply rather than reading `.call` off a page-owned
function, and it walks both the listener copy and the dispatch path by index,
so the page-mutable iterator protocol is not on the path at all. Two host
reads sit on the same footing and only those two: every realm answer crosses
through the captured `String`, and the two action builders serialise their
result through the captured `JSON.stringify` — because that result *is* the
action's answer, and a replaced serialiser could otherwise hand the host
well-formed JSON claiming an action applied.

Every host action whose answer decides `applied`, `default_prevented`, a
navigation, a reset or a submit goes through **one capability-guarded bridge**
in the base: it mints the lexically captured `Event` rather than the global a
page can replace, walks the closure-owned dispatcher rather than an element's
`dispatchEvent`, and answers from hidden state rather than a property lookup a
page can shadow or redefine. The global is non-writable, non-configurable and
non-enumerable, its capability is minted per realm — children included — and
never leaves the host, and a wrong capability is a `TypeError` with no
fallback, because a fallback to the public methods is the hole. The DOM's own
`click`, `submit` and `reset` take the same closure-owned path.

A registration is its own record, so the same callback on two targets or
under two types is two registrations and removing one says nothing about the
other; a listener removed during a dispatch stays silent for it even if the
same function is registered again. A stop flag set before a dispatch holds for
that dispatch and is cleared when it completes. Dispatching something that is
not an event throws a real `TypeError`. A listener type is a string, whatever
the page passed. `isTrusted` is false everywhere, because these events are
synthesized by an agent; `timeStamp` reads the clock the realm already had. Losses: listener
options (`capture`, `once`, `passive`, `signal`), the capture phase and so
`eventPhase === 1`, `handleEvent` objects, `composedPath`, the interface
constants, `relatedTarget`, `srcElement`, every typed event interface beyond
`Event` and `CustomEvent`, and any reporting of an exception a listener threw.

`classList` and `CustomEvent` (`element-api-design-0.0.1.md`): a page can
work with the class attribute it could already read. `classList` holds no
tokens — the attribute is the state, and `add`, `remove`, `toggle` and
`contains` reparse it on every call and write it back, so a list held across a
direct write to `class` answers about the attribute as it is now and a
mutation through it never drops what was written directly. It is the same
object every time, allocated on first access and dying with the element: a
handle, not a cache. `value` is the attribute's own string, with a setter that
writes what it is given, while the token reads stay parsed over whatever
raggedness that leaves. A call that changes nothing writes nothing and does not
advance the revision, which diverges from the standard deliberately: the
revision is what gates a caller's action, and a spurious one costs a
re-snapshot for a change that did not happen. The revision advances per
...
a script-free child realm has neither.
...
`pending`, `discarded_total` and `last_cause` — one of two fixed words,
`caller_override` or `bridge_failure`. The realm refuses to retain an address
longer than it may keep, and the host bounds what crosses in **UTF-8 bytes**
before any parse, join or fetch and then applies the same `navigation_url`
check a caller's address passes; every refusal answers one fixed redacted
reason. Taking an intent is a boundary that can fail: an evaluation that
failed propagates, malformed bridge output is a typed `intent_bridge_failed`,
and neither is ever reported as "no intent". Because such an evaluation may
have left the slot full, the target is **poisoned** — the next boundary empties
the slot and discards what it held before any page code runs on that realm
again, so what a page asked for through a failed bridge is never applied
later. Every page-initiated navigation writes one bounded ledger record like
any other: a fixed operation naming the kind (`page.navigate.assign`,
`…replace`, `…reload`), a fixed outcome, and an origin only when the address
parsed into one. A link is judged after its candidate is: an abandoned
candidate — one that raised another intent before anything could observe it —
records `superseded`, only the document the caller can see records
`committed`, and a chain refused at its cap records no commit at all. An explicit `target.navigate`,
...
| **current, `2d57ce864002…`** — `-text-answer-court` (29/29, new), `-probe-truthfulness-repaired` (**25/25, the first passing run of that court ever committed**) and the four live-guard verification receipts `-element-tag-verification` (52/52), `-signature-integrity-verification` (34/34), `-registry-brand-verification` (15/15) and `-property-shape-verification` (22/22). Verified on this exact binary in the same round: `-attribute-fact` (154/154), `-element-tag` (52/52), `-signature-integrity` (34/34), `-property-shape` (22/22), `-registry-brand` (15/15), `-capture-declarations` (8/8), `-probe-truthfulness` (25/25), `-host-answers` (9/9), `-downloads` (21/21), `-snapshot-schema` (13/13), `-copy-on-write` (23/23), `-readonly-profile` (28/28), `-form` (179/179), `-frame-actions` (182/182), `-page-navigation` (80/80), `-lifecycle` (53/53), `-job-deadline` (42/42), `-element-api` (28/28), `-dataset` (15/15), `-event-fidelity` (62/62), `-timers` (68/68) and `-child-frames` (82/82) | the host that never loses an answer to half a character (`5535eaa`) | `snapshot_script` cuts with a `cut(raw, n)` built only from a string's own `length`, index reads and `+=`: it stops one character early rather than splitting a surrogate pair and replaces an unpaired surrogate with U+FFFD, at all six sites that cut a page-derived string. **A surrogate pair straddling the 256-unit cut used to lose the whole snapshot** to a bare `internal`, reachable by ordinary content at an unlucky offset. The frozen court is `text-answer-court.py`; `-text-answer-court-falsification` and `-text-answer-court-falsification-amended` are both the build before it, `cc8ebfa4a4dc…`, at **13 of 29**, the second recorded separately after one criterion was made per-field rather than per-document. **Cost zero on every axis**: both shims unchanged and child-frame M1/M2 identical on both allocators, because the change lives in a host script. **No pin moved.** `-option-value` is the read-only probe of the round after it, on this same binary, and `-element-scaling` and `-attribute-store` are the read-only probes of the round after that. `-element-scaling` measures **1,329.6 bytes per element** on the system arm and **1,279.3** on the arena arm, against **1,097.4 / 1,029.5** before round C: the element-fact rounds bought **+232.2 bytes per element (21%)** while the per-attribute cost never moved, and the 16 MiB realm limit fell from 14,991 to 12,370 elements. Its two comparison receipts `-element-scaling-round-c` and `-element-scaling-pre-round-c` carry the hashes of binaries **rebuilt off the canonical path**, so by the rule at the head of this table those hashes are **not** the historical `e9e07111…` and `0da1c6b11553…` and must not be read as those builds' tokens — runtime memory does not depend on the build path, which is what makes the comparison valid and the hash match irrelevant. `-attribute-store` asked whether a page can write through the `__attrs` accessor and answered **no**: in ten arms every write lands where its honest `setAttribute` twin lands, so F1 and F2 stay closed. G1, G3, P6 and G6 stay open |
| `cc8ebfa4a4dc…`, superseded — `-attribute-fact` (154/154, new), `-text-answer` (probe), `-text-answer-court-falsification` and `-text-answer-court-falsification-amended` (both 13/29, the pre-change arm) | the host whose attribute facts are its own (`a0482ed`, round D, candidate E) | one record per element holds the tag **and** the attribute map, read through two non-writable globals; the normalisation is rebuilt from index reads and `+=`. **F1 and F2 close**: a declared `method="post"` is no longer submitted as a GET and a `target="somewhere"` link is no longer activated, on every route — two selective `toLowerCase` lies, a `Map.prototype.get` that lies, a direct `el.__attrs` write, and an arm that replaces and deletes both readers. `__attrs` keeps an ignoring landing setter, without which a page's own write throws and answers `target_crashed`. Base shim 33,290 → **33,886** (+596, ceiling +600); child-frame M1 +2,896/+2,736 and M2 +20,272/+21,552 against ceilings of +3,200 and +23,000. **Five pins re-frozen, each with its old value kept**: `property-shape`'s `window` (`113:5899bf6e` → `114:5d05836a`) and `Element.prototype` (`40:26312e4` → `41:1c3f518d`, `__attrs` becoming a page-observable accessor), `signature-integrity`'s base pin, `registry-brand`'s N3 hash, and `element-tag`'s cost group. `-attribute-fact-falsification` is the build before it, `e9e071115461…`, at **126 of 154** |
| `e9e071115461…`, superseded — `-element-tag` (52/52, new), `-attribute-fact-falsification` (126/154, the pre-change arm) | the host whose tag is its own (`a26aea3`, round C) | a closure-owned `WeakMap` from element to tag, written through the already-captured `weakMapSet` and read through a non-writable, non-configurable `__mcsTag`; all seven host-script tag reads ask it, `role()` included, so **a `div` a page renames is not offered as a node at all** rather than merely refused at the download. **F5 closes.** Nothing is taken from the page: `tagName` stays a writable data property, so `el.tagName = "A"` still succeeds and still reads back. Base shim 32,898 → **33,290** (+392, ceiling +400) with the main shim untouched; child-frame M1 +1,696/+1,952 and M2 +11,936/+13,040 against +2,048 and +14,336. **Two pins re-frozen**: `property-shape`'s `window` (`112:156a0f8b` → `113:5899bf6e`) — and its `Element.prototype` row deliberately did **not** move, which was the check that this round added no prototype member — and `signature-integrity`'s base pin. A third, `registry-brand`'s N3 shim hash, was reported rather than taken and ruled afterwards. `-element-tag-falsification` is the build before it, `0da1c6b11553…`, at **38 of 52** |
| `e3cbf79eb64b…`, discarded — `-fail-open-triage-priced-arm` | a throwaway build that priced a page-unreachable tag (`efb60ab`) | **Not a shipped host.** It existed to price candidate C by building it rather than estimating: +293 base bytes, child-frame M1 +1,776 and M2 +12,496 on the system arm. It was measured and then reverted, the tree restored and the binary rebuilt to `0da1c6b11553…`. Kept in the register because its receipt is committed evidence and a hash in the tree with no row is worse than one with a row that says what it was |
| `0da1c6b11553…`, superseded — `-signature-integrity` (34/34, new), `-fail-open-triage` (probe), `-element-tag-falsification` (38/52, the pre-change arm) | the host whose approval binds (`7b9e11a`) | the approval signature concatenates with a literal separator instead of `join`; `urlOf` is rebuilt from `"" + raw`, a string's own `length`, index reads and `+=`, with no `replace` and no global `String`; the fragment test is `value[0] === "#"`; and `REALM_PROBE_JS` concatenates seven booleans with `":"` and calls neither `join` nor `String`. **F3, F4 and the court probe close.** Measuring before freezing changed the design: patching `String.prototype.replace` defeated the interlock exactly as `join` did, so a court frozen on the `join` route alone would have passed over a live F4. **One frozen court was amended, chronologically**: `probe-truthfulness`'s S2 counted `String(` occurrences as a proxy for "seven questions are still asked", and this round had to remove every one; it now names the seven questions and requires six literal separators. `-signature-integrity-falsification` is the build before it, `ba46420bb1e6…`, at **19 of 34** |
| `ba46420bb1e6…`, superseded — `-snapshot-schema` (13/13, new), `-uncaptured-intrinsic` (probe), `-signature-integrity-falsification` (19/34, the pre-change arm) | the host that refuses a snapshot answer it cannot read (`579820e`) | the snapshot parse stops defaulting: every field is required at both parse sites, the `node_0` default is gone, and a malformed answer is `target_crashed` with `reason: "snapshot_schema"` and the offending field, rather than a silent default. Four act scripts gained the matching `unusable` branch. The `-uncaptured-intrinsic` probe on this same binary is what named the five fail-opens the four rounds above close, and measured that **18 of 43 intrinsic replacements change nothing at all**, `JSON.stringify` among them, which is H1 holding and re-measured |
| `ce371f78e38c…`, superseded — `-registry-brand` (15/15, new) | the host whose revision registry is its own (`ed9169e`) | the registry is installed as a branded, defined property: `bump` and `setTo` verify a per-realm brand minted in `Realm::new`, and an installer that finds the name occupied answers `internal` with `reason: "registry_occupied"` instead of adopting what it finds. **Reached by naming a global first rather than by replacing an intrinsic**, which is why it closed a hole the intrinsic work could not. Three implementation faults were found and fixed inside the round — an unminted brand interpolated as a hole, an installer that ran before `documentElement` existed, and a third construction site the two call sites missed |
| `05aa12f7cf6d…`, superseded — `-capture-declarations` (8/8, new), `-probe-truthfulness` (**21/25, deliberately failing**) | the host with the H3 capture guard, before the realm-probe repair (`210f740`, court frozen at `38a9207`) | the declared-capture guard requires every capture to be referenced or reserved in writing, with `arrayIndexOf` on the reserved list at zero uses. **`-probe-truthfulness` is the frozen pre-repair arm and is meant to read 21 of 25**: it was frozen from `realm-probe-audit-0.0.1.md` before the repair and fails until it lands. **The repair landed at `a229c13` and no passing receipt was ever committed for it** — see the note below this table |
| `18f935f5fe3e…`, superseded — `-host-answers` (9/9, new) | the host whose answers are not the page's to write (`4b7d42c`, H1) | all 24 `JSON.stringify` calls in the host's raw-string scripts route through `__mcsJson`, a non-writable global holding the intrinsic captured before any page script runs. A page that replaced `JSON.stringify` could otherwise hand the agent a document that does not exist and send the host to a URL the agent never referenced. **One sentence in this round's audit was later corrected**: "the host re-checks the node kind itself" was false when written — `not_a_link` was refused only when the realm declined an href — proven in `uncaptured-intrinsic-audit-0.0.1.md` §4, and true again since `958f5c0` made the tag the host's |
| `e168722ca9b2…`, superseded — `-copy-on-write` (23/23), `-property-shape` (22/22, new at `13821f8`) | the host whose fork cannot silently fall back (`092b50e`) | `source_for_fork` takes the source's writer lock and re-reads the sealed record from disk under it, with **no fallback**: an unreadable source is a store error, never host memory. The property-shape guard beside it pins the shape of every own property of the objects the shim installs onto, so a refactor cannot change what a page can see without ruling it |
| `952226eeb470…`, falsification only — `-copy-on-write-falsification` (**22/23**, `code: null`, `child_on_disk: true`) | a deliberately rebuilt host carrying the fallback that was removed (`092b50e`) | **Not a shipped host.** The fallback defect was found by review rather than by the court, so the court's F20 criterion was proven falsifiable by rebuilding the defect and watching it fail. Kept as the record of that proof |
| `4e8f753817b9…`, superseded — `-copy-on-write` (20/20, new; the receipt was later rerun and overwritten at 23/23 on `e168722ca9b2…`) | the host that forks a profile copy-on-write (`7cbf052`) | a fork copies the sealed record rather than sharing it, and a persistent parent may fork an ephemeral child. This is the one row whose original receipt no longer exists in the tree: the file was rerun in the next round, which is why the convention now says a committed receipt is never refreshed |
| `8d5da2a706ae…`, superseded — `-downloads` (21/21, new), `-frame-actions` (182/182) | the host that serves downloads over the control protocol (`6374d62`) | sink C: permission answered where it is used, per-profile count and byte budgets, a jar-copy fetch with a fresh budget, sha256 and base64, and a `reported_name` bounded at 255 bytes and carried verbatim. The transport stress that preceded it (`b00dd3a`) **produced no receipt** — see the note below this table |
| `8ff70b9f26c1…`, superseded — **twenty-nine receipts were rerun on this exact binary**, including `-readonly-profile` (28/28, new), `-profile` (**90/94**, see below), `-tag-name-query` (37/37), `-abort-signal-timeout` (25/25), `-abort-signal-surface` (37/37), `-abort-signal` (27/27), `-listener-options` (30/30), `-capture-phase` (36/36), `-passive-listener` (30/30), `-event-target` (24/24), `-attribute-name` (34/34), `-error-class` (27/27), `-page-error-redaction` (23/23), `-element-view` (23/23), `-active-element` (16/16), `-dataset` (15/15), `-event-view` (11/11), `-event-fidelity` (62/62), `-child-frames` (82/82), `-shim-footprint` (18/18), `-form` (179/179), `-frame-actions` (182/182), `-page-navigation` (80/80), `-element-api` (28/28), `-lifecycle` (53/53), `-timers` (68/68), `-job-deadline` (42/42), `-frame-realm` (62/62) and `-cdp-frame-tree` (64/64). `-navigation` was **not rerun** | the host where an open can promise not to write | `session.open` takes a `mode`, and a readonly session refuses writes for itself with `unsupported_capability` / `session_read_only` — **deliberately not** the failed-commit `commit_failed`, so a client can tell *you asked for this* from *the store broke*. The mode is **per session and not persisted**: it dies with the session, and a later plain session on the same profile writes again. An **ephemeral** profile cannot be opened readonly — it has nothing to read — and `profile.create` does **not** take the mode, which a contract negative now pins. **The shape moved twice, and both moves are in the record**: a `mode` on `profile.create` cannot reach a profile adopted at startup, and a court written for two live sessions cannot pass a host that allows one per profile. Both constraints appeared only when the code was written. `-readonly-profile-falsification` is the build before it, `420cdf5b82bf…`, at **4 of 6** — the count grows to 28 once the argument exists, which the receipt's limitations say. **`-profile` reads 90 of 94, not the 92 the previous receipt carried, and that is not this slice**: the same four fail on the pre-implementation binary too, measured, and the two new ones are the store's `resident_delta` (1.69 MB against a 1 MiB bound) drifting with the machine's working set. The **latch criterion passes on both arms**, which is what this slice had to protect. `-navigation` stays as it was; no cap or floor moved. G1, G3, P6 and G6 stay open |
| `420cdf5b82bf…`, superseded — **twenty-eight receipts were rerun on this exact binary**: `-tag-name-query` (37/37, new), `-class-name-query-hold` (**5/36, deliberately failing** — see below), `-abort-signal-timeout` (25/25), `-abort-signal-surface` (37/37), `-abort-signal` (27/27), `-passive-listener` (30/30), `-capture-phase` (36/36), `-listener-options` (30/30), `-event-target` (24/24), `-attribute-name` (34/34), `-error-class` (27/27), `-page-error-redaction` (23/23), `-element-view` (23/23), `-active-element` (16/16), `-dataset` (15/15), `-event-view` (11/11), `-event-fidelity` (62/62), `-child-frames` (82/82), `-shim-footprint` (18/18), `-form` (179/179), `-frame-actions` (182/182), `-page-navigation` (80/80), `-element-api` (28/28), `-lifecycle` (53/53), `-timers` (68/68), `-job-deadline` (42/42), `-frame-realm` (62/62) and `-cdp-frame-tree` (64/64). `-navigation` was **not rerun** and still carries `abd1ed744721…` | the host a legacy script can call by tag | `getElementsByTagName(name)` exists on one member of `Node`, so a page has it on both `document` and any element: descendants in document order, the element itself excluded, case-insensitive names, `'*'` equal to `querySelectorAll('*')`, a detached subtree served, and an empty or unparseable name answering **empty rather than throwing**. It unlocks no capability — a page could always call `querySelectorAll(tag)` — and buys compatibility with legacy script that fails at the call rather than the concept. **It adds no authority, reentrancy or lifecycle**, which the court pins rather than assumes: inside a dispatch it runs one listener and changes nothing, and each call allocates its own array and owns nothing after. **Two losses are pinned as criteria**, because a page meets them as wrong answers rather than errors: the result is a plain array and not a live `HTMLCollection`, inherited from `querySelectorAll`, and a namespace-ish name such as `a:b` answers empty where a browser matches an element literally called that. Cost is the ruled **+576**: main slack 62,016 to **62,592**, 2,944 left of 65,536, with M1 232,298 and M2 1,624,588 unmoved and no child paying anything. `-tag-name-query-falsification` is the build before it, `e8f85cf59354…`, at **6 of 37** with `passed: false`. **`-class-name-query-hold` is not a regression**: `getElementsByClassName` stopped at its cost gate and stays unimplemented, so its frozen 36-criterion court reads 5 of 36 by design and is kept as the verification gate for whenever 1,408 bytes are reclaimed. `-profile` stays 90 of 94, its four D6 checks narrow, **older-build evidence not rerun here**. Navigation stays cross-batch narrow, no cap or floor moved. G1, G3, P6 and G6 stay open |
| `e8f85cf59354…`, superseded — **twenty-six receipts were rerun on this exact binary**: `-abort-signal-timeout` (25/25, new), `-abort-signal-surface` (37/37), `-abort-signal` (27/27), `-passive-listener` (30/30), `-capture-phase` (36/36), `-listener-options` (30/30), `-event-target` (24/24), `-attribute-name` (34/34), `-error-class` (27/27), `-page-error-redaction` (23/23), `-element-view` (23/23), `-active-element` (16/16), `-dataset` (15/15), `-event-view` (11/11), `-event-fidelity` (62/62), `-child-frames` (82/82), `-shim-footprint` (18/18), `-form` (179/179), `-frame-actions` (182/182), `-page-navigation` (80/80), `-element-api` (28/28), `-lifecycle` (53/53), `-timers` (68/68), `-job-deadline` (42/42), `-frame-realm` (62/62) and `-cdp-frame-tree` (64/64). `-navigation` was **not rerun** and still carries `abd1ed744721…` | the host where a timeout cannot eat the page's clock | `AbortSignal.timeout(ms)` exists and fires once with a `TimeoutError` `DOMException`, dropping the listeners bound to it. **The reserve is the design**: it spends from the page's own 64-entry timer table and refuses once that table holds 16, so **a page keeps 48 slots for `setTimeout`** and timeout signals are capped at 16, the refusal being a `RangeError` it can catch. The quota **adds no state of its own** — it reads the table already there — which is the shape that made it fit: an earlier counter-and-limit version measured 66,704 against the 65,536 bound and this one measures **62,016, with 3,520 to spare**, M1 232,298 and M2 1,624,588 unmoved, so no child pays. **The standing guard was amended by ruling, not bypassed**: no host path may abort a page's signal, except the timer `timeout()` itself created, which may abort that signal and nothing else — the court holds a controller's signal and a long timeout signal across the host's own drain and requires both still un-aborted. The surface court's `timeout`-absent pin was amended to require it, which is what that pin was for. `-abort-signal-timeout-falsification` is the build before it, `536ad23ed12a…`, at **6 of 25** with `passed: false`, taken against the pre-implementation sources. Two of the new court's criteria were **amended after their first run and the amendments are recorded** in §7.3 of the audit: both measured the fixture rather than the rule. `-profile` stays 90 of 94, its four D6 checks narrow, **older-build evidence not rerun here**. Navigation stays cross-batch narrow, no cap or floor moved. G1, G3, P6 and G6 stay open |
| `536ad23ed12a…`, superseded — **twenty-five receipts were rerun on this exact binary**: `-abort-signal-surface` (37/37, new), `-abort-signal` (27/27), `-passive-listener` (30/30), `-capture-phase` (36/36), `-listener-options` (30/30), `-event-target` (24/24), `-attribute-name` (34/34), `-error-class` (27/27), `-page-error-redaction` (23/23), `-element-view` (23/23), `-active-element` (16/16), `-dataset` (15/15), `-event-view` (11/11), `-event-fidelity` (62/62), `-child-frames` (82/82), `-shim-footprint` (18/18), `-form` (179/179), `-frame-actions` (182/182), `-page-navigation` (80/80), `-element-api` (28/28), `-lifecycle` (53/53), `-timers` (68/68), `-job-deadline` (42/42), `-frame-realm` (62/62) and `-cdp-frame-tree` (64/64). `-navigation` was **not rerun** and still carries `abd1ed744721…` | the host whose signals answer for themselves | the signal is an `EventTarget`, `abort` fires on it once, `reason` exists with an `AbortError` `DOMException` by default, `throwIfAborted()` throws it, `AbortSignal.abort(reason)` mints an aborted signal, and `onabort` is a real handler that replaces and clears. **Two of those closed silent failures**: `abort("because")` used to take the argument and discard it, and `onabort` used to stick and never fire. The reentrancy is ruled and pinned — the page's own `abort()` dispatches where it was called, `first > abort listener > back > third`, and an abort listener's exception stays local — with the standing constraint that **no host path may abort a page's signal**. `reason`, the handler and the brand stay in closure-owned maps, and **the handle's key set did not grow**. **Costs no child anything**: M1 stays 232,298 and the child delta is **0**, M2 1,626,636; the whole price is main-only slack, 54,560 to **60,032**, leaving 5,504 of 65,536 — which is now the binding bound rather than the child floor, with M1 headroom still 13,462. `AbortSignal.timeout()` is **pinned absent** as the deferred candidate, so taking it later amends that court. `-abort-signal-surface-falsification` is the build before it, `9975bb659414…`, at **13 of 37** with `passed: false`. `-profile` stays 90 of 94, its four D6 checks narrow, **older-build evidence not rerun here**. Navigation stays cross-batch narrow, no cap or floor moved. G1, G3, P6 and G6 stay open |
| `9975bb659414…`, superseded — **twenty-four receipts were rerun on this exact binary**: `-abort-signal` (27/27, new), `-passive-listener` (30/30), `-capture-phase` (36/36), `-listener-options` (30/30), `-event-target` (24/24), `-attribute-name` (34/34), `-error-class` (27/27), `-page-error-redaction` (23/23), `-element-view` (23/23), `-active-element` (16/16), `-dataset` (15/15), `-event-view` (11/11), `-event-fidelity` (62/62), `-child-frames` (82/82), `-shim-footprint` (18/18), `-form` (179/179), `-frame-actions` (182/182), `-page-navigation` (80/80), `-element-api` (28/28), `-lifecycle` (53/53), `-timers` (68/68), `-job-deadline` (42/42), `-frame-realm` (62/62) and `-cdp-frame-tree` (64/64). `-navigation` was **not rerun** and still carries `abd1ed744721…` | the host whose signals it minted itself | `AbortController` and `AbortSignal` exist in the main extension, while the base holds two closure-owned `WeakSet`s — the signals it minted and the ones aborted — read through captured methods. A signal the host did not mint is a `TypeError`, so **a page object never reaches a record and a page getter never runs inside the walk**: the audit measured that the naive version let a page's getter execute mid-dispatch and remove a listener the same dispatch was about to run. Abort removes, a pre-aborted signal registers nothing, an abort during a dispatch stops a listener not yet run without unwinding one that has, and it composes with `capture`, `once` and `handleEvent`. The one-shot handle gained exactly one entry, `signals`, and **the court now pins the handle's exact key set** so the next widening fails a criterion instead of passing unnoticed. **M1 229,322 to 232,298 and M2 to 1,624,588** — the ruled +2,976 per child — against unchanged floors, **13,462 bytes of M1 headroom**, main-only slack 54,560. `-abort-signal-falsification` is the build before it, `91352a77f024…`, at **4 of 27** with `passed: false`, taken against the pre-implementation sources so its source-inventory criteria fail honestly. The court's S8 criterion was **amended after its first run and the amendment is recorded**: its expected sequence was written wrong, the implementation was right, and §9.1 of the audit says so. `-profile` stays 90 of 94, its four D6 checks narrow, **older-build evidence not rerun here**. Navigation stays cross-batch narrow, no cap or floor moved. G1, G3, P6 and G6 stay open |
| `91352a77f024…`, superseded — **twenty-three receipts were rerun on this exact binary**: `-passive-listener` (30/30, new), `-capture-phase` (36/36), `-listener-options` (30/30), `-event-target` (24/24), `-attribute-name` (34/34), `-error-class` (27/27), `-page-error-redaction` (23/23), `-element-view` (23/23), `-active-element` (16/16), `-dataset` (15/15), `-event-view` (11/11), `-event-fidelity` (62/62), `-child-frames` (82/82), `-shim-footprint` (18/18), `-form` (179/179), `-frame-actions` (182/182), `-page-navigation` (80/80), `-element-api` (28/28), `-lifecycle` (53/53), `-timers` (68/68), `-job-deadline` (42/42), `-frame-realm` (62/62) and `-cdp-frame-tree` (64/64). `-navigation` was **not rerun** and still carries `abd1ed744721…` | the host a page can no longer refuse an action from | a listener that declares itself `{passive: true}` can no longer cancel. **This one takes power away from the page**: before this build, a link whose own click listener promised not to cancel and then called `preventDefault()` **refused the agent's navigation**; now the navigation happens, and a plain listener still refuses it, so exactly one route closed. A passive listener also can no longer cancel the outer event from inside a nested dispatch, while the nested event stays cancellable in its own right. The flag lives on the record and the window is opened and closed around each invocation, so passivity never leaks between listeners, phases or dispatches. Ruled and pinned rather than left ambiguous: a **late** `preventDefault` after the dispatch has returned still writes the flag — the host has already read its answer, so a page only misleads itself — and **no event type is passive unless the page says so**. **M1 228,922 to 229,322 and M2 to 1,603,756** — the ruled +400 per child, the cheapest rung of the ladder — against unchanged floors, **16,438 bytes of M1 headroom**, main-only slack 46,864. `-passive-listener-falsification` is the build before it, `112b303b5edd…`, at **16 of 30** with `passed: false`. `-profile` stays 90 of 94, its four D6 checks narrow, **older-build evidence not rerun here**. Navigation stays cross-batch narrow, no cap or floor moved. G1, G3, P6 and G6 stay open |
| `112b303b5edd…`, superseded — **twenty-two receipts were rerun on this exact binary**: `-capture-phase` (36/36, new), `-listener-options` (30/30), `-event-target` (24/24), `-attribute-name` (34/34), `-error-class` (27/27), `-page-error-redaction` (23/23), `-element-view` (23/23), `-active-element` (16/16), `-dataset` (15/15), `-event-view` (11/11), `-event-fidelity` (62/62), `-child-frames` (82/82), `-shim-footprint` (18/18), `-form` (179/179), `-frame-actions` (182/182), `-page-navigation` (80/80), `-element-api` (28/28), `-lifecycle` (53/53), `-timers` (68/68), `-job-deadline` (42/42), `-frame-realm` (62/62) and `-cdp-frame-tree` (64/64). `-navigation` was **not rerun** and still carries `abd1ed744721…` | the host whose events finally travel in both directions | capture, then the target, then bubbling, with the window outermost and `eventPhase` reading 1, 2, 3. A **non-bubbling event now captures down** and still does not bubble, which is the standard and is not what this host did before. `capture` joined listener identity, so a removal with the wrong flag removes nothing and the two flags are two registrations — the old behaviour deleted a listener it should have kept. The page-visible change is on the record: a capture listener can `stopPropagation()` before the target's own listener runs, so a page can keep its own target listeners from seeing a synthesized action. **The host's decision does not move**: stopping propagation leaves the navigation happening and preventing the default still refuses it, both measured through a real `target.act`. **M1 227,258 to 228,922 and M2 to 1,600,956** — the ruled +1,664 per child exactly — against unchanged floors, **16,838 bytes of M1 headroom**, main-only slack 46,464. Getting there took a correction worth recording: the first build carried four explanatory comments in the base and cost **+2,944 per child**, so I measured the split — code +1,664, prose +1,280 — condensed the prose, found the saving is **quantized rather than linear** (four blocks trimmed to one line each gave back only 256 bytes), and moved the invariants into the audit and the court instead, where they cost a page nothing. `-capture-phase-falsification` is the build before it, `799895aab740…`, at **16 of 36** with `passed: false`. `-profile` stays 90 of 94, its four D6 checks narrow, **older-build evidence not rerun here**. Navigation stays cross-batch narrow, no cap or floor moved. G1, G3, P6 and G6 stay open |
| `799895aab740…`, superseded — **twenty-one receipts were rerun on this exact binary**: `-listener-options` (30/30, new), `-event-target` (24/24), `-attribute-name` (34/34), `-error-class` (27/27), `-page-error-redaction` (23/23), `-element-view` (23/23), `-active-element` (16/16), `-dataset` (15/15), `-event-view` (11/11), `-event-fidelity` (62/62), `-child-frames` (82/82), `-shim-footprint` (18/18), `-form` (179/179), `-frame-actions` (182/182), `-page-navigation` (80/80), `-element-api` (28/28), `-lifecycle` (53/53), `-timers` (68/68), `-job-deadline` (42/42), `-frame-realm` (62/62) and `-cdp-frame-tree` (64/64). `-navigation` was **not rerun** and still carries `abd1ed744721…` | the host where a listener can ask to run once | the first listener-options rung: the options reach `addListener` at last — three call sites forwarded two arguments — `{once:true}` spends the registration after one run, on a page's dispatch and on **two of the agent's own clicks**, and an object with `handleEvent` is called with itself as the receiver instead of being dropped in silence. The handler is resolved **at registration**, so the walk never reads a property off a page object; a page that swaps the method afterwards keeps what it registered, which is a deliberate divergence. `capture`, `passive` and `signal` are deferred and unpinned. **M1 225,626 to 227,258 and M2 to 1,589,308** against unchanged floors, **18,502 bytes of M1 headroom**, main-only slack 44,768 inside 65,536. The audit priced this rung at +1,392 per child and it came in at **+1,632**; the difference was measured rather than shrugged at — the three explanatory comments I added to the base cost **384 bytes per child** on their own, and the code alone is +1,248. `-listener-options-falsification` is the build before it, `6177450e0cc0…`, at **14 of 30** with `passed: false`. `-profile` stays 90 of 94, its four D6 checks narrow, **older-build evidence not rerun here**. Navigation stays cross-batch narrow, no cap or floor moved. G1, G3, P6 and G6 stay open |
| `6177450e0cc0…`, superseded — **twenty receipts were rerun on this exact binary**: `-event-target` (24/24, new), `-attribute-name` (34/34), `-error-class` (27/27), `-page-error-redaction` (23/23), `-element-view` (23/23), `-active-element` (16/16), `-dataset` (15/15), `-event-view` (11/11), `-event-fidelity` (62/62), `-child-frames` (82/82), `-shim-footprint` (18/18), `-form` (179/179), `-frame-actions` (182/182), `-page-navigation` (80/80), `-element-api` (28/28), `-lifecycle` (53/53), `-timers` (68/68), `-job-deadline` (42/42), `-frame-realm` (62/62) and `-cdp-frame-tree` (64/64). `-navigation` was **not rerun** and still carries `abd1ed744721…` | the host where the event bus finally has its name | `EventTarget` exists, a node is one, and the chain reads `Element > Element > Node > EventTarget > Object`; `new EventTarget()` is a bus and a subclass receives. The behaviour was always there — the base keys listeners by object, so a borrowed method on a plain object was already a working bus, and that still works. **The slice is main-only**: the base does not grow, the one-shot handle is not asked for anything new, and **M1 225,626 and M2 1,577,884 do not move at all**, so a child realm pays nothing; the whole price is 2,352 bytes of main-only slack, 40,576 to 42,928 inside 65,536. `window` stays outside the chain by ruling and keeps its own three methods. The three authority properties are re-measured under the constructor and hold: a forged parent reaches no real listener, the reserved focus type moves nothing, and a page's spy on `EventTarget.prototype.dispatchEvent` does not take the host's own `target.act`. `-event-target-falsification` is the build before it, `f19f20f01aff…`, at **11 of 24** with `passed: false`, where the name is a `ReferenceError`. Listener options, `handleEvent` objects and `AbortController` stay unserved and unpinned, as a separate candidate. `-profile` stays 90 of 94, its four D6 checks narrow, **older-build evidence not rerun here**. Navigation stays cross-batch narrow, no cap or floor moved. G1, G3, P6 and G6 stay open |
| `f19f20f01aff…`, superseded — **nineteen receipts were rerun on this exact binary**: `-attribute-name` (34/34, new), `-error-class` (27/27), `-page-error-redaction` (23/23), `-element-view` (23/23), `-active-element` (16/16), `-dataset` (15/15), `-event-view` (11/11), `-event-fidelity` (62/62), `-child-frames` (82/82), `-shim-footprint` (18/18), `-form` (179/179), `-frame-actions` (182/182), `-page-navigation` (80/80), `-element-api` (28/28), `-lifecycle` (53/53), `-timers` (68/68), `-job-deadline` (42/42), `-frame-realm` (62/62) and `-cdp-frame-tree` (64/64). `-navigation` was **not rerun** and still carries `abd1ed744721…` | the host that refuses to author a name it would still faithfully read | one guard in the `setAttribute` funnel, so `id`, `className`, `dataset`, `classList`, `toggleAttribute` and the method itself validate together: a name outside the ASCII `Name` production throws `InvalidCharacterError` with `code` 5, before the name is lowercased, and a `dataset` key with a dash before a lowercase letter throws `SyntaxError`. `removeAttribute` and `toggleAttribute(false)` stay lenient and `classList` keeps its own vocabulary, both by ruling. **The parser's ten awkward names still arrive, read back and deep-clone** — that is what the clone's internal copy bought, and the court pins it. The thrown message carries neither the offending name nor the value. **M1 224,762 to 225,626 and M2 to 1,577,884** — 864 bytes per child — against unchanged floors of 245,760 and 1,720,320, leaving **20,134 bytes of M1 headroom**, with main-only slack 40,576 inside 65,536. `-attribute-name-falsification` is the build before it, `7545d393b69c…`, at **12 of 34** with `passed: false`, where every bad name is accepted. **One unreproduced test failure is recorded rather than hidden**: a single `cargo test` run reported 53 passed and 1 failed in 0.42 s immediately after a rebuild, its name lost to the filter I ran it through. A stability review since then is **72 runs with no failure** — 12 plain, 10 serial under `--no-fail-fast -- --test-threads=1` with full output kept, 5 build-then-test, 5 parallel, 20 of the single suspect test and 20 more of it four ways concurrent. The suspect is named by timing rather than by a reproduction: the suite's whole 2.02 s lives in `surface::tests::failed_shows_unmap_the_frame_and_reap_the_child` (every other group is 0.40 s or less), and a run that ends in 0.42 s is what an early failure in that test looks like, because its final `/bin/sleep` arm is the two seconds. Its `/usr/bin/yes` arm is a real race between a child's first bytes and the parent's read. That is an inference from the duration, not a diagnosis, and the failure stays on the record as unexplained. `-profile` stays 90 of 94, its four D6 checks narrow, **older-build evidence not rerun here**. Navigation stays cross-batch narrow, no cap or floor moved. G1, G3, P6 and G6 stay open |
| `7545d393b69c…`, superseded — **eighteen receipts were rerun on this exact binary**: `-error-class` (27/27, new), `-page-error-redaction` (23/23), `-element-view` (23/23), `-active-element` (16/16), `-dataset` (15/15), `-event-view` (11/11), `-event-fidelity` (62/62), `-child-frames` (82/82), `-shim-footprint` (18/18), `-form` (179/179), `-frame-actions` (182/182), `-page-navigation` (80/80), `-element-api` (28/28), `-lifecycle` (53/53), `-timers` (68/68), `-job-deadline` (42/42), `-frame-realm` (62/62) and `-cdp-frame-tree` (64/64). `-navigation` was **not rerun** and still carries `abd1ed744721…` | the host whose own refusals are the standard ones | the selector engine's four entry points throw a captured `DOMException` named `SyntaxError` with `code` 12, and `removeChild` one named `NotFoundError` with `code` 8. The constructor is captured at base load, so a page that replaces `globalThis.DOMException` or `globalThis.Error` first changes nothing — **measured, not argued**: before this build, a page replacing `Error` made the base throw the page's own constructor. `classList`, the dispatch guard, storage, `cloneNode` and the timers keep exactly what they had, by ruling, so this host carries two error vocabularies and not three. **M1 224,458 to 224,762 and M2 to 1,571,836** against unchanged floors of 245,760 and 1,720,320 — **20,998 bytes of M1 headroom** — with main-only slack 38,800 inside 65,536; the price is 304 bytes per child, exactly what the audit measured on two earlier baselines. `-error-class-falsification` is the build before it, `30004da4d050…`, at **9 of 27** with `passed: false`. The redaction still holds over the new class: `-page-error-redaction` is 23/23 here and the error court's own E10 re-checks it, so the class change is not mistaken for a leak repair. `-profile` stays 90 of 94, its four D6 checks narrow, **older-build evidence not rerun here**. Navigation stays cross-batch narrow, no cap or floor moved. G1, G3, P6 and G6 stay open |
| `30004da4d050…`, superseded — **seventeen receipts were rerun on this exact binary**: `-page-error-redaction` (23/23, new), `-element-view` (23/23), `-active-element` (16/16), `-dataset` (15/15), `-event-view` (11/11), `-event-fidelity` (62/62), `-child-frames` (82/82), `-shim-footprint` (18/18), `-form` (179/179), `-frame-actions` (182/182), `-page-navigation` (80/80), `-element-api` (28/28), `-lifecycle` (53/53), `-timers` (68/68), `-job-deadline` (42/42), `-frame-realm` (62/62) and `-cdp-frame-tree` (64/64). `-navigation` was **not rerun** and still carries `abd1ed744721…` | the host that will not repeat what a page said | a page's exception message stops at the catch site: `details.engine_error` now says one of two host-authored words and nothing else, so a page that throws a string built from a form value no longer hands that value to the caller. Costs nothing anywhere — **M1 224,458, M2 1,569,707 and main-only slack 38,496 are byte-identical to the build before**, because the change is host-side Rust and touches no shim source. `-page-error-redaction-falsification` is the build before it, `4a5836f43b38…`, at **17 of 23** with `passed: false`, failing R1, R2 and R8 on both allocators and nothing else. The court was frozen one commit ahead of the fix and its criteria, fixtures and opaque tokens are unchanged since. The selector engine is **exactly as it was**: this was verified without the `DOMException` candidate, so the redaction is the reason the values are gone, not a side effect. `-profile` stays 90 of 94, its four D6 checks narrow, **older-build evidence not rerun here**. Navigation stays cross-batch narrow, no cap or floor moved. G1, G3, P6 and G6 stay open |
| `4a5836f43b38…`, superseded — **sixteen receipts were rerun on this exact binary**: `-element-view` (23/23), `-active-element` (16/16), `-dataset` (15/15), `-event-view` (11/11), `-event-fidelity` (62/62), `-child-frames` (82/82), `-shim-footprint` (18/18), `-form` (179/179), `-frame-actions` (182/182), `-page-navigation` (80/80), `-element-api` (28/28), `-lifecycle` (53/53), `-timers` (68/68), `-job-deadline` (42/42), `-frame-realm` (62/62) and `-cdp-frame-tree` (64/64). `-navigation` was **not rerun** and still carries `abd1ed744721…` | the host where a copy is a copy and not a re-authoring | the clone writes the element's own attribute map directly, so it carries names the parser accepted and authoring would not. It costs nothing — it removes a call: **M1 unchanged at 224,457**, M2 1,569,707 against the frozen floors, and main-only slack 38,496 inside 65,536. The criterion went in one commit ahead of the code and **passes on every committed build by construction**, because nothing validates attribute names yet; `-clone-internal-copy-falsification` is therefore not a build in this history but a throwaway carrying a validator beside the old re-authoring clone, at **21 of 23** with `passed: false`, where the fixture's own script dies on both allocators. That receipt says so in an `annotation` added by hand; no check result was changed. `-profile` stays 90 of 94, its four D6 checks narrow, **older-build evidence not rerun here**. Navigation stays cross-batch narrow, no cap or floor moved. G1, G3, P6 and G6 stay open |
| `eaf4de9b68b6…`, superseded — **sixteen receipts were rerun on this exact binary**: `-element-view` (23/23), `-active-element` (16/16), `-dataset` (15/15), `-event-view` (11/11), `-event-fidelity` (62/62), `-child-frames` (82/82), `-shim-footprint` (18/18), `-form` (179/179), `-frame-actions` (182/182), `-page-navigation` (80/80), `-element-api` (28/28), `-lifecycle` (53/53), `-timers` (68/68), `-job-deadline` (42/42), `-frame-realm` (62/62) and `-cdp-frame-tree` (64/64). `-navigation` was **not rerun** and still carries `abd1ed744721…` | the host where a page can copy a subtree | the last of the five page-only additions: **M1 unchanged at 224,458**, M2 1,569,708 against the frozen floors, and main-only slack 38,240 inside 65,536. `-element-view-falsification` is the build before it, `744153d18294…`, at 19 of 23 with `passed: false`. `-profile` stays 90 of 94, its four D6 checks narrow, **older-build evidence not rerun here**. Navigation stays cross-batch narrow, no cap or floor moved. G1, G3, P6 and G6 stay open |
| `744153d18294…`, superseded — sixteen receipts stood on it: `-element-view` (21/21), `-active-element` (16/16), `-dataset` (15/15), `-event-view` (11/11), `-event-fidelity` (62/62), `-child-frames` (82/82), `-shim-footprint` (18/18), `-form` (179/179), `-frame-actions` (182/182), `-page-navigation` (80/80), `-element-api` (28/28), `-lifecycle` (53/53), `-timers` (68/68), `-job-deadline` (42/42), `-frame-realm` (62/62) and `-cdp-frame-tree` (64/64). `-navigation` was **not rerun** and still carries `abd1ed744721…` | the host where a page can toggle an attribute | a main-only compatibility fix: **M1 unchanged at 224,458**, M2 1,569,708 against the frozen floors, and main-only slack 33,568 inside 65,536. `-element-view-falsification` is the build before it, `1e472770aa50…`, at 17 of 21 with `passed: false`. `-profile` stays 90 of 94, its four D6 checks narrow, **older-build evidence not rerun here**. Navigation stays cross-batch narrow, no cap or floor moved. G1, G3, P6 and G6 stay open |
| `1e472770aa50…`, superseded — sixteen receipts stood on it: `-element-view` (19/19), `-active-element` (16/16), `-dataset` (15/15), `-event-view` (11/11), `-event-fidelity` (62/62), `-child-frames` (82/82), `-shim-footprint` (18/18), `-form` (179/179), `-frame-actions` (182/182), `-page-navigation` (80/80), `-element-api` (28/28), `-lifecycle` (53/53), `-timers` (68/68), `-job-deadline` (42/42), `-frame-realm` (62/62) and `-cdp-frame-tree` (64/64). `-navigation` was **not rerun** and still carries `abd1ed744721…` | the host where a page can enumerate an element's attribute names | a main-only compatibility fix: **M1 unchanged at 224,458**, M2 1,569,708 against the frozen floors, and main-only slack 32,720 inside 65,536. `-element-view-falsification` is the build before it, `8546428d23d9…`, at 17 of 19 with `passed: false`, where the fixture's own script dies on the missing method. `-profile` stays 90 of 94, its four D6 checks narrow, **older-build evidence not rerun here**. Navigation stays cross-batch narrow, no cap or floor moved. G1, G3, P6 and G6 stay open |
| `8546428d23d9…`, superseded — sixteen receipts stood on it: `-active-element` (16/16), `-element-view` (19/19), `-dataset` (15/15), `-event-view` (11/11), `-event-fidelity` (62/62), `-child-frames` (82/82), `-shim-footprint` (18/18), `-form` (179/179), `-frame-actions` (182/182), `-page-navigation` (80/80), `-element-api` (28/28), `-lifecycle` (53/53), `-timers` (68/68), `-job-deadline` (42/42), `-frame-realm` (62/62) and `-cdp-frame-tree` (64/64). `-navigation` was **not rerun** and still carries `abd1ed744721…` | the host where an agent's action moves focus and a page's does not | the focus state is base-side, so this slice costs child realms: **M1 221,514 to 224,458 and M2 to 1,569,708**, against unchanged floors of 245,760 and 1,720,320 — **21,302 bytes of M1 headroom** — with main-only slack 31,872 inside 65,536. `-active-element-falsification` is the build before it, `0ac5a9a7dbbf…`, at **2 of 16**: there is no `activeElement` there at all. `-profile` stays 90 of 94, its four D6 checks narrow, **older-build evidence not rerun here**. Navigation stays cross-batch narrow, no cap or floor moved. G1, G3, P6 and G6 stay open |
| `0ac5a9a7dbbf…`, superseded — fifteen receipts stood on it: `-element-view` (19/19), `-dataset` (15/15), `-event-view` (11/11), `-event-fidelity` (62/62), `-child-frames` (82/82), `-shim-footprint` (18/18), `-form` (179/179), `-frame-actions` (182/182), `-page-navigation` (80/80), `-element-api` (28/28), `-lifecycle` (53/53), `-timers` (68/68), `-job-deadline` (42/42), `-frame-realm` (62/62) and `-cdp-frame-tree` (64/64). `-navigation` was **not rerun** and still carries `abd1ed744721…` | the host where a page's `closest` answers instead of throwing | a compatibility fix, not a capability: **M1 unchanged at 221,514**, M2 1,549,100, and a main-only page costs 28,128 against the `origin/main` baseline — **608 bytes** more than before, inside the 65,536 slack. `-element-view-falsification` is the build before it, `64e77ee292d7…`, at 17 of 19 with `passed: false`: there the fixture's own script dies where it calls `closest`, on both allocators. `-profile` stays 90 of 94, its four D6 checks narrow, **older-build evidence not rerun here**. Navigation stays cross-batch narrow, no cap or floor moved. G1, G3, P6 and G6 stay open |
| `64e77ee292d7…`, superseded — fifteen receipts stood on it: `-dataset` (15/15), `-element-view` (19/19), `-event-view` (11/11), `-event-fidelity` (62/62), `-child-frames` (82/82), `-shim-footprint` (18/18), `-form` (179/179), `-frame-actions` (182/182), `-page-navigation` (80/80), `-element-api` (28/28), `-lifecycle` (53/53), `-timers` (68/68), `-job-deadline` (42/42), `-frame-realm` (62/62) and `-cdp-frame-tree` (64/64). `-navigation` was **not rerun** and still carries `abd1ed744721…` | the host where `dataset` is built when a page reads it | a **per-element** cost left the base: the marginal bytes per child node fall from 2,084 to 1,253 on the system allocator and 2,038 to 1,202 on the arena, against a frozen gate of 1,600. **M1 221,514 and M2 1,549,100** against unchanged floors of 245,760 and 1,720,320 — **24,246 bytes of M1 headroom** — and a main-only page costs 27,488 above the `origin/main` baseline, *less* than before, because a page now pays for `dataset` only on the elements it touches. `-dataset-falsification` is the build before it, `e9d7273c310e…`, at 12 of 15: its per-element gate fails on both allocators. `-profile` stays 90 of 94, its four D6 checks narrow, **older-build evidence not rerun here**. Navigation stays cross-batch narrow, no cap or floor moved. G1, G3, P6 and G6 stay open |
| `e9d7273c310e…`, superseded — fourteen receipts stood on it: `-element-view` (19/19), `-event-view` (11/11), `-event-fidelity` (62/62), `-child-frames` (82/82), `-shim-footprint` (18/18), `-form` (179/179), `-frame-actions` (182/182), `-page-navigation` (80/80), `-element-api` (28/28), `-lifecycle` (53/53), `-timers` (68/68), `-job-deadline` (42/42), `-frame-realm` (62/62) and `-cdp-frame-tree` (64/64). `-navigation` was **not rerun** and still carries `abd1ed744721…` | the host after C2a: `contains` moved and calls the base's own helper | **M1 230,106 and M2 1,609,244** against unchanged floors of 245,760 and 1,720,320 — **15,654 bytes of M1 headroom** — and a main-only page costs 32,192 above the `origin/main` baseline, inside the 65,536 slack. `-element-view-audit-comparison` is that court run against the build before it. It scores 19 of 19 there too, so it **separates nothing and is not a falsification**: its receipt carries `passed: null` and `verdict: non_discriminating` rather than a pass, because the inventory criteria read the shipped sources, `contains` behaves the same whichever realm installs it, and the divergence is visible only through a court-only probe that is part of the binary under test — an older binary answers with its own older probe. That receipt was renamed and its verdict set by hand as a provenance fix; no criterion, measurement or check result was changed, and the design record carries the reasoning in §12.4. **This round has no falsification receipt**, and says so rather than presenting a passing run as one. **Navigation's** failures stand as measured, no cap or floor moved. `-profile` stays 90 of 94, its four D6 checks narrow, **older-build evidence not rerun here**. G1, G3, P6 and G6 stay open |
| `98fd6418e8ef…`, superseded — fourteen receipts stood on it: `-element-view` (17/17), `-event-view` (11/11), `-event-fidelity` (62/62), `-child-frames` (82/82), `-shim-footprint` (18/18), `-form` (179/179), `-frame-actions` (182/182), `-page-navigation` (80/80), `-element-api` (28/28), `-lifecycle` (53/53), `-timers` (68/68), `-job-deadline` (42/42), `-frame-realm` (62/62) and `-cdp-frame-tree` (64/64). `-navigation` was **not rerun** and still carries `abd1ed744721…` | the host where a page-only DOM member lives only in the realm that can call it | ten members left the base: **M1 230,506 and M2 1,612,044** against unchanged floors of 245,760 and 1,720,320, so **M1 headroom is 15,254 bytes**, and a main-only page costs 305,544 against the `origin/main` baseline's 273,512 — 32,032 more, inside the 65,536 slack. `-element-view-audit-comparison` (then named `-element-view-falsification`) was the build before it, `7552dbfa52b5…`, at 16 of 17: four of that court's criteria read the shipped sources rather than the binary, so only the runtime half falsifies on an old binary, and the design record says so rather than letting the number read as a weak falsification. **Navigation's** repeated failures stand as measured, no cap was moved. `-profile` stays 90 of 94, its four D6 checks narrow, **older-build evidence not rerun here**. G1, G3, P6 and G6 stay open |
| `7552dbfa52b5…`, superseded — thirteen receipts stood on it: `-event-view` (11/11), `-event-fidelity` (62/62), `-child-frames` (82/82), `-shim-footprint` (18/18), `-form` (179/179), `-frame-actions` (182/182), `-page-navigation` (80/80), `-element-api` (28/28), `-lifecycle` (53/53), `-timers` (68/68), `-job-deadline` (42/42), `-frame-realm` (62/62) and `-cdp-frame-tree` (64/64). `-navigation` was **not rerun** and still carries `abd1ed744721…` | the host where the page-facing view of an event lives only in the realm that can read it | the ten accessors left the base: **M1 236,938 and M2 1,657,068** against unchanged floors of 245,760 and 1,720,320, so **M1 headroom went from 2,630 to 8,822 bytes**, and a main-only page costs 307,256 against the `origin/main` baseline's 273,512 — 33,744 more, inside the 65,536 slack. `-event-view-falsification` is the build before it, `b11e6363d248…`, at 10 of 11: there every child realm still carried the ten accessors it can never read. Earlier falsifications are kept for `2b6d985fe682…` (8/28), `abd1ed744721…` (28/36), `a91bdf2c85b7…` (38/40), `ec20ffab6af3…` (46/54) and `44ad1aa47263…` (56/62). **Navigation's** repeated failures stand as measured, no cap was moved, and the attribution's reading and limits are in `event-fidelity-design-0.0.1.md` §17-18. `-profile` stays 90 of 94, its four D6 checks narrow, **older-build evidence not rerun here**. G1, G3, P6 and G6 stay open |
| `b11e6363d248…`, superseded — twelve receipts stood on it: `-event-fidelity` (62/62), `-child-frames` (82/82), `-shim-footprint` (18/18), `-form` (179/179), `-frame-actions` (182/182), `-page-navigation` (80/80), `-element-api` (28/28), `-lifecycle` (53/53), `-timers` (68/68), `-job-deadline` (42/42), `-frame-realm` (62/62) and `-cdp-frame-tree` (64/64). `-navigation` was **not rerun** and still carries `abd1ed744721…` | the host whose privileged dispatch is built only from intrinsics captured before page script | **M1 243,130 and M2 1,700,412** against the unchanged floors of 245,760 and 1,720,320 — **2,630 bytes of M1 headroom under the floor**, 19,014 under the cap, and this slice has now spent nearly all of the shim split's margin. `-event-fidelity-intrinsics-falsification` is the build before it, `44ad1aa47263…`, at 56 of 62: on that build a handler that replaced the array iterator hid the ancestor's listener from the host's walk, one that patched `WeakMap.prototype.get` reached the hidden state and forced `applied: false`, and one that replaced `JSON.stringify` made the host report an action result reading `role: "fabricated"`. Earlier falsifications are kept for `2b6d985fe682…` (8/28), `abd1ed744721…` (28/36), `a91bdf2c85b7…` (38/40) and `ec20ffab6af3…` (46/54). **Navigation's** repeated failures stand as measured, no cap was moved, and the attribution's reading and limits are in `event-fidelity-design-0.0.1.md` §17–18. `-profile` stays 90 of 94, its four D6 checks narrow, **older-build evidence not rerun here**. G1, G3, P6 and G6 stay open |
| `44ad1aa47263…`, superseded — twelve receipts stood on it: `-event-fidelity` (54/54), `-child-frames` (82/82), `-shim-footprint` (18/18, against the `origin/main` baseline `7f2429d96df8…`), `-form` (179/179), `-frame-actions` (182/182), `-page-navigation` (80/80), `-element-api` (28/28), `-lifecycle` (53/53), `-timers` (68/68), `-job-deadline` (42/42), `-frame-realm` (62/62) and `-cdp-frame-tree` (64/64). `-navigation` was **not rerun** and still carries `abd1ed744721…` | the host whose action dispatch a page cannot reach | **M1 236,074 and M2 1,650,236** against the unchanged floors of 245,760 and 1,720,320 and the unmoved caps. `-event-fidelity-authority-falsification` is the build before the bridge, `ec20ffab6af3…`, at 46 of 54: on it an own property shadowing `defaultPrevented` cancelled a host action, a forged prototype getter cancelled it, a replaced global `Event` broke the action outright, and a replaced `dispatchEvent` let the host report an applied action whose handler never ran. Three earlier falsifications are kept: `2b6d985fe682…` at 8 of 28, `abd1ed744721…` at 28 of 36 and `a91bdf2c85b7…` at 38 of 40. **Navigation's** repeated failures on `a91bdf2c85b7…` stand as measured, no cap was moved, and the read-only attribution (`event-fidelity-design-0.0.1.md` §17–18) found identical owner growth on both builds, zero `realm_malloc_bytes` growth and every owner zero after close, with the divergence appearing during the candidate build and released at the swap — the route stays **cross-batch narrow on the default allocator**. That attribution reports no live `Event` or listener owner bytes, because no such owner exists, and samples neither libmalloc allocated and resident nor the arena counters. `-profile` stays 90 of 94, its four D6 checks narrow, **older-build evidence not rerun here**. G1, G3, P6 and G6 stay open |
| `ec20ffab6af3…`, superseded — three receipts stood on it: `-event-fidelity` (40/40), `-child-frames` (82/82) and `-shim-footprint` (18/18, against the `origin/main` baseline `7f2429d96df8…`). Every other receipt in the directory names an **older build** and is presented as that build's evidence, not this one's: `-element-api` (28/28), `-page-navigation` (80/80), `-lifecycle` (53/53), `-job-deadline` (42/42), `-timers` (68/68), `-frame-actions` (182/182), `-form` (179/179), `-frame-realm` (62/62), `-cdp-frame-tree` (64/64) and `-navigation` (90/90) all carry `abd1ed744721…` | the host with one faithful `Event`, one correct dispatcher and per-registration listener identity | **M1 233,530 and M2 1,632,428** against the unchanged floors of 245,760 and 1,720,320 and the unmoved caps of 262,144 and 1,835,008. Three falsifications are kept, each naming the build it judged: `-event-fidelity-falsification` is the build before the slice, `2b6d985fe682…`, at 8 of 28 against the court as first frozen; `-event-fidelity-round-two-falsification` is round one, `abd1ed744721…`, at 28 of 36 against the court the second audit extended; `-event-fidelity-typeerror-falsification` is `a91bdf2c85b7…` at 38 of 40, which fails only the criterion that insists on a real `TypeError`. The 28-of-28 event receipt that stood on `abd1ed744721…` was the first round's court and is superseded, not current. **Navigation was not rerun for this row**: its repeated failures on `a91bdf2c85b7…` (88, 90, 89 of 90) stand as measured, no cap was moved, and the read-only attribution (`event-fidelity-design-0.0.1.md` §17–18) found owner growth identical across 128 navigations on both builds, `realm_malloc_bytes` growth zero and every owner zero after close, with the divergence appearing during the candidate build and released at the swap — so the route stays **cross-batch narrow on the default allocator**, now with a measured reason. That attribution could not report live `Event` or listener owner bytes, because no such owner exists, and did not sample libmalloc allocated or resident or the arena counters. `-profile` stays 90 of 94, its four D6 checks narrow, **older-build evidence not rerun here**. G1, G3, P6 and G6 stay open |
| `abd1ed744721…`, superseded as the current row — the ten receipts listed above still carry it: `-event-fidelity` (28/28), `-element-api` (28/28), `-shim-footprint` (18/18, against the `origin/main` baseline `7f2429d96df8…`), `-child-frames` (82/82), `-page-navigation` (80/80), `-lifecycle` (53/53), `-job-deadline` (42/42), `-timers` (68/68), `-frame-actions` (182/182), `-form` (179/179), `-frame-realm` (62/62), `-cdp-frame-tree` (64/64), `-navigation` (90/90) | the host with one faithful `Event` and one correct dispatcher | `-event-fidelity-falsification` is the build before it, `2b6d985fe682…`, at 8 of 28. This slice is the first to spend the shim split's margin deliberately: the base grew 3,230 source bytes, M1 moved 221,657 to **232,938** and M2 1,549,324 to 1,628,284, both still under the 245,760 and 1,720,320 floors, leaving **12,822 bytes of M1 headroom under the floor** and 29,206 under the cap. `-navigation` stays **cross-batch narrow on the default allocator**. `-profile` stays 90 of 94, its four D6 checks narrow, **older-build evidence not rerun here**. G1, G3, P6 and G6 stay open |
| `2b6d985fe682…`, superseded — every court in that row was a committed receipt on that binary: `-element-api` (28/28), `-shim-footprint` (18/18, against the `origin/main` baseline `7f2429d96df8…`), `-child-frames` (82/82), `-page-navigation` (80/80), `-lifecycle` (53/53), `-job-deadline` (42/42), `-timers` (68/68), `-frame-actions` (182/182), `-form` (179/179), `-frame-realm` (62/62), `-cdp-frame-tree` (64/64), `-navigation` (90/90) | the host after the classList root audit: a live view, the same object each time, the standard `value`, and a null dictionary that is not a crash | `-element-api-falsification` is the build the audit judged, `a0b455c0e05a…`, at 16 of 28: it fails every criterion the audit added and no other. The floors hold with the extension larger again — M1 unchanged at 221,657, a main-only page 286,488 against 273,512, inside the 65,536 slack. `-navigation` stays **cross-batch narrow on the default allocator**. `-profile` stays 90 of 94, its four D6 checks narrow, **older-build evidence not rerun here**. G1, G3, P6 and G6 stay open |
| `a0b455c0e05a…`, superseded — every court in that row was a committed receipt on that binary: `-element-api` (18/18), `-shim-footprint` (18/18, against the `origin/main` baseline `7f2429d96df8…`), `-child-frames` (82/82), `-page-navigation` (80/80), `-lifecycle` (53/53), `-job-deadline` (42/42), `-timers` (68/68), `-frame-actions` (182/182), `-form` (179/179), `-frame-realm` (62/62), `-cdp-frame-tree` (64/64), `-navigation` (90/90) | the host with `classList` and `CustomEvent` in the main extension | `-element-api-falsification` is the build before it, `878a86a847d3…`, at 2 of 18. The shim floors are re-passed with the extension larger: M1 is unchanged at 221,657 because a child compiles none of it, and a main-only page costs 284,904 against 273,512 — 11,392 more, inside the 65,536 slack. `-navigation` stays **cross-batch narrow on the default allocator**. `-profile` stays 90 of 94, its four D6 checks narrow, **older-build evidence not rerun here**. G1, G3, P6 and G6 stay open |
| `878a86a847d3…`, superseded — every court in that row was a committed receipt on that binary: `-shim-footprint` (18/18, against the `origin/main` baseline `7f2429d96df8…` built from `1bbdf00`), `-child-frames` (82/82), `-page-navigation` (80/80), `-lifecycle` (53/53), `-job-deadline` (42/42), `-timers` (68/68), `-frame-actions` (182/182), `-form` (179/179), `-frame-realm` (62/62), `-cdp-frame-tree` (64/64), `-navigation` (90/90) | the host with the per-realm shim split | the recovery is measured against the exact `origin/main` binary in one court run, both allocators: M1 261,961 to 221,657, M2 1,831,451 to 1,549,323, a main-only page 288 bytes cheaper, and 28 child realms about 1.0 MB (system) and 2.6 MB (arena) smaller in process footprint, every candidate host below every baseline host. Binary +1,472 bytes, incremental build 5.46 s to 6.12 s. `-navigation` stays **cross-batch narrow on the default allocator**. `-profile` stays 90 of 94, its four D6 checks narrow, **older-build evidence not rerun here**. G1, G3, P6 and G6 stay open |
| `5a8afd0d9737…`, superseded — **committed receipts on that binary**: `-page-navigation` (80/80) and `-child-frames` (82/82). The same-binary regression runs — `-lifecycle` (53/53), `-job-deadline` (42/42), `-timers` (68/68), `-frame-actions` (182/182), `-form` (179/179), `-frame-realm` (62/62), `-cdp-frame-tree` (64/64), `-navigation` (90/90) — were run against this exact binary as **scratch results that were not retained**; their committed receipts still name `099b46da4199…` and are that build's evidence, not this one's | the host after the audit-truth ruling: an abandoned chain candidate is `superseded`, never `committed` | three falsification receipts, each rerun on this court so they can be compared: `-page-navigation-falsification` is the build before the slice, `0fc0cf97f606…` at 30 of 72; `-page-navigation-audit-falsification` is the build the root audit judged, `24bb19416d9e…` at 56 of 76; `-page-navigation-ledger-falsification` is the build before this ruling, `ec7f9768f19b…` at 76 of 80, failing exactly the four criteria this ruling added. `-child-frames` is unchanged from the round before — this fix adds no shim source — so M1 stays 261,962 against 262,144: **182 bytes of headroom**, and **further shim growth is blocked until a separate architecture slice reduces what every realm compiles**. `-navigation` stays **cross-batch narrow on the default allocator**: its differential soak criterion has failed in other batches on the same code, and a passing batch is not a repair. `-profile` stays 90 of 94, its four D6 checks narrow, and is **older-build evidence not rerun here**. G1, G3, P6 and G6 stay open |
| `ec7f9768f19b…`, superseded, and its regression numbers were scratch results that were not retained — `-page-navigation` (76/76), `-lifecycle` (53/53), `-job-deadline` (42/42), `-timers` (68/68), `-frame-actions` (182/182), `-child-frames` (82/82), `-form` (179/179), `-frame-realm` (62/62), `-cdp-frame-tree` (64/64), `-navigation` (90/90 on rerun) | the host after the page-navigation root audit: a take that fails is a failure, the page's address is bounded at both ends, and every page-initiated navigation is in the ledger | two falsification receipts are kept. `-page-navigation-falsification` is the build before the slice, `0fc0cf97f606…`, at 30 of 68. `-page-navigation-audit-falsification` is the build the root audit judged, `24bb19416d9e…`, at 56 of 72: it fails **every** criterion the audit added and no other. `-child-frames` was rerun because this slice moved its numbers again: M1 261,962 against an unmoved 262,144, so **M1 has 182 bytes of headroom**. `-navigation` failed its `tail_excess_slope` differential soak criterion at 89 of 90 in the first batch and passed 90 of 90 on rerun, on this binary and on the one before it; the route stays **cross-batch narrow on the default allocator** and this is not a repair. `-profile` stays 90 of 94, its four D6 checks narrow, and is **older-build evidence not rerun here**. G1, G3, P6 and G6 stay open |
| `24bb19416d9e…`, superseded, and its regression numbers were scratch results that were not retained — `-page-navigation` (50/50), `-lifecycle` (53/53), `-job-deadline` (42/42), `-timers` (68/68), `-frame-actions` (182/182), `-child-frames` (82/82), `-form` (179/179), `-frame-realm` (62/62), `-cdp-frame-tree` (64/64), `-navigation` (90/90) | the host with page-initiated navigation | `-page-navigation-falsification` is the build before it, `0fc0cf97f606…`, where the frozen court passes only 20 of 46 and is kept as that record. `-child-frames` was rerun because this slice moved its numbers: M1 261,354 and M2 1,827,196 live owner bytes against unmoved caps of 262,144 and 1,835,008, so **M1 has 790 bytes of headroom**. Navigation's **90 of 90 is this batch**: its differential soak criteria have failed in other batches on the same code, so the route stays **cross-batch narrow on the default allocator**. `-profile` stays 90 of 94, its four D6 checks narrow, and is **older-build evidence not rerun here**. G1, G3, P6 and G6 stay open |
...
- **The realm-probe repair still has no receipt of its own, and now the court
  ...
  and stays exactly where it is. The repair landed at `a229c13`, which committed
  no receipt and whose binary hash is written nowhere, so that round's evidence
  cannot be reconstructed and this register does not pretend otherwise.
  `-probe-truthfulness-repaired` is the first **passing** run ever committed,
  25 of 25 on the current binary `2d57ce864002…`, and it says in the file that
  it is today's status rather than the repair round's. No verification receipt
  accompanies it: this court carries no live guard, only behaviour and the shape
  of the probe's source. Note the inversion worth remembering — here the
  *plainly named* receipt is the failing arm and the suffixed one passes, the
  opposite of the `-falsification` pattern everywhere else.
- **Receipt names do not derive mechanically from court names.** Seven pairs
  differ — `child-frame`/`-child-frames`, `timer`/`-timers`,
  `host-answer`/`-host-answers`, `capture-declaration`/`-capture-declarations`,
  `frame-action`/`-frame-actions`, `class-name-query`/`-class-name-query-hold`,
  and `retention-court.py`, which is a shared library and not a court. All are
  naming, none is a missing receipt. One receipt of 107,
  `-job-deadline-falsification`, is referenced by no document.
  `-text-answer-court` is the only receipt that keeps the `-court` suffix.
  **Nothing is renamed**: a committed receipt is history, and the record is here
  so the next one does not copy the exception.
...
and links the historical one:

| guard | historical receipt | verification receipt |
|---|---|---|
| `element-tag-court.py`'s rebased cost equalities | `-element-tag` (52/52 on `e9e071115461…`) | `-element-tag-verification` |
