...
Ten page-only DOM members — `firstChild`, `lastChild`, `parentElement`,
`appendChild`, `remove`, `innerText`, `defaultValue`, `focus`, `blur` and
`submit` — are installed by the main extension on the same prototypes, each
the base's own implementation moved rather than rewritten. No host script names
any of them, which a court re-derives from the shipped sources rather than
taking on trust, so no child realm compiles them. Four other page-only members
— `contains`, `matches`, `removeAttribute` and `replaceChildren` — stay in the
base by ruling, because each is written in terms of base internals and moving
