...
`contains` parse it and write it back, so the list can never disagree with
what it describes. A call that changes nothing writes nothing and does not
...
ledger, error or counter. Losses: iteration, index access, `item`, `replace`,
