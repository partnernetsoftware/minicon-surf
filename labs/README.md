...
| `native-dom-surface` | the native route's G3 surface process: a minimal direct-Cocoa window child over a bounded binary IPC (codec shared with the host as a library) | prototype — native court 106/110: attach, real input, CDP continuity, reaped detach and owners to zero hold; post-hide host footprint and slope over the pre-registered caps; the paired attribution put them on the freed frame kept by the default zone, the surface-owned mmap frame region removed that and its variance, and the residual small-block churn still fails S2 and S3, so G3 stays open |
