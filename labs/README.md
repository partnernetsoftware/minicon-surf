...
| `surface-court` | standalone macOS native-surface probes for G3 (plain control, direct Cocoa via objc2, winit + softbuffer) with a colour-bar fixture read back from the own window | measured — frozen S1–S9: real windows attach and detach with the owner at 0 → 1 → 0, but AppKit keeps about 10 MB after hide plus a per-cycle residual, so no in-process candidate is eligible; surface-process design adopted: see `native-dom-surface` and the native G3 court (106/110, narrow) |
| `native-dom-surface` | the native route's G3 surface process: a minimal direct-Cocoa window child over a bounded binary IPC (codec shared with the host as a library) | prototype — native court 106/110: attach, real input, CDP continuity, reaped detach and owners to zero hold; post-hide host footprint and slope over the pre-registered caps; the paired attribution put them on the freed frame kept by the default zone, the surface-owned mmap frame region removed that and its variance, and the residual small-block churn still fails S2 and S3, so G3 stays open; headless by default since the owner rule (real windows only under `--visual` plus `MINICON_SURF_ALLOW_VISIBLE_COURT=1`, once, by hand) |
