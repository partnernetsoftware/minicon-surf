...
| ME3 | process-metric shape: report `{pid, type, name}` per child in the process-per-target host and check the sum equals the court's descendants-only RSS. **Done**: per-child opaque identity, pid, role, lifecycle state and generation with resident and footprint, reconciled with the shared sampler at four stages inside a fixed bracket; no field claimed equivalent to Electron's | `labs/lightpanda` host | M2 vocabulary; G1 open |
